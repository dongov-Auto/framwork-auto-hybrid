...
public class PayUnsuccessfulTest extends BaseTestMobile {
    ...
    private YourOrderFailedObjectPage yourOrderFailedPage;
...
//        vehicleDetailsPage.then_Verify_Caculated_Estimated_Total_Successfully();
        ...
        Assert.assertTrue(myRoadTaxStorePage.then_verify_my_road_tax_screen_dispalyed());
        myRoadTaxStorePage.scrollDown();
        yourOrderFailedPage = myRoadTaxStorePage.clickToCancelPayment();
        Assert.assertTrue(yourOrderFailedPage.then_verify_your_order_failed_screen_displayed());
        confirmPayPage=yourOrderFailedPage.when_click_to_try_again();
        myRoadTaxStorePage = confirmPayPage.clickToCheckoutButton();
        Assert.assertTrue(myRoadTaxStorePage.then_verify_my_road_tax_screen_dispalyed());
        myRoadTaxStorePage.scrollDown();
        yourOrderFailedPage = myRoadTaxStorePage.clickToCancelPayment();
        roadTaxPage=yourOrderFailedPage.when_click_to_return_to_home_page();
        Assert.assertTrue(roadTaxPage.then_Verify_How_It_Work_Link_Displayed());
